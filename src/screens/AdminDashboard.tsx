...
export const AdminDashboard = () => {
  ...
  const handleApprove = async (id: string) => {
    const { error } = await supabase
      ...
      .eq('id', id);
    if (error) Alert.alert('Error', error.message);
    else Alert.alert('Success', 'Entry approved');
  };
    ...
    <View style={styles.entryCard}>
      ...
      <View style={styles.entryStats}>
        <View>
          ...
          <Text style={styles.entryStatValue}>₹{item.net_profit?.toFixed(2) ?? 'Calculating...'}</Text>
        </View>
        {!item.approved && (
...
const styles = StyleSheet.create({
  ...
  entryCard:       { backgroundColor: Colors.surface, padding: Spacing.md, borderRadius: 12, marginBottom: Spacing.md, borderWidth: 1, borderColor: Colors.border },
